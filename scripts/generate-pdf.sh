...
<body>
  ...
  <!-- PORTADA -->
  <div class="cover">
    <span class="badge">SPECIFICATION V1.2 • FAIL-CLOSED</span>
  ...
  <p><code>PreToolUse</code> intercepta cada llamada a herramienta del agente antes de ejecutarse (<code>scripts/hooks/guard-holdouts.sh</code>) y bloquea cualquier escritura o borrado sobre <code>tests/holdout/</code> o <code>.holdout.sha256</code>. Si el payload de entrada no es JSON parseable, el hook falla cerrado de inmediato en vez de asumir que la llamada es inofensiva. <code>Stop</code> (<code>scripts/hooks/stop-gate.sh</code>) impide que el agente declare terminado un turno si <code>just gauntlet-fast</code> no sale en verde. <code>PostToolUse</code> (<code>scripts/hooks/lint-on-edit.sh</code>) da feedback rápido de ESLint tras cada <code>Edit</code>/<code>Write</code> sobre un <code>.ts</code>, sin bloquear nada.</p>

  <div class="callout callout-warn">
    <strong>Solo <code>exit 2</code> bloquea <code>PreToolUse</code>/<code>Stop</code> en Claude Code.</strong> Los dos hooks de seguridad de este harness usaron <code>exit 1</code> desde su creación hasta que se detectó y corrigió en una sesión posterior (ver <code>docs/adr/0001-hooks-de-seguridad-usan-exit-2.md</code>) — no bloqueaban nada, pese a que el código parecía correcto. Los hooks de <strong>seguridad</strong> (<code>PreToolUse</code>, <code>Stop</code>) fallan CERRADO; los de <strong>conveniencia</strong> (<code>PostToolUse</code>) fallan ABIERTO. Ver <code>AGENTS.md</code> para la tabla completa de clasificación.
  </div>
...
├── .claude/
│   ├── agents/
│   │   ├── generator.md            # Subagente: único rol con Edit/Write
│   │   └── reviewer.md             # Subagente: revisa diffs con contexto limpio, sin Edit/Write
│   └── settings.json                # Hooks de agente: PreToolUse, Stop, PostToolUse (Capa 1)
...
├── docs/
│   ├── adr/                        # Decisiones arquitectónicas: contexto + consecuencias
│   └── progress.md                 # Handoff entre sesiones (se sobrescribe, no se acumula)
...
│   │   ├── lint-on-edit.sh         # PostToolUse: feedback rápido de ESLint (falla abierto)
